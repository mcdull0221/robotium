...
import com.nurotron.ble_ui.GAIA_Library.Gaia.AsrResult;
...
public class GaiaCommand
{
     ...
     */
    public GaiaCommand(byte[] source, boolean isGATT)
    {
        if (isGATT)
            buildGATTCommand(source, source.length);
        else
    ...
    /**
     * Build a GAIA command payload from a byte sequence from GATT.
     * Created by Chenxi
     * @param source Array of bytes to build the command from.
     * @param source_length Number of bytes from the array to use.
     */
    private void buildGATTCommand(byte[] source, int source_length){

        int payload_length = source_length - Gaia.OFFS_GATT_PAYLOAD;
        mVendorId = shortFromByteArray(source, Gaia.OFFS_GATT_VENDOR_ID);
        mCommandId = shortFromByteArray(source, Gaia.OFFS_GATT_COMMAND_ID);

        if (payload_length > 0)
        {
            mCommandPayload = new byte[payload_length];
            for (int i = 0; i < payload_length; ++i)
                mCommandPayload[i] = source[i + Gaia.OFFS_GATT_PAYLOAD];
        }
    }
    ...
    /**
     * Get the status byte from the payload of an acknowledgement packet.
     * @return The customized status code as defined in Gaia.CustomStatusCode.
     */
    public int getCustomStatusCode()
    {
        ...
        return Gaia.CustomStatusCode.valueOf(mCommandPayload[0]);
    }
